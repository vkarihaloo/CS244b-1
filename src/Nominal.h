...
class Nominal {
 public:
  ...
  const Nominal<UnitType, RepType>& operator=(
      const Nominal<UnitType, RepType>& v) {
    value_ = v.value_;
    return *this;
  }
...
class Ordinal : public Nominal<UnitType, RepType> {
 public:
  ...
  Ordinal<UnitType, RepType> operator+(
      const Ordinal<UnitType, RepType>& other) {
  ...
  const Ordinal<UnitType, RepType>& operator++() {
    ++Nominal<UnitType, RepType>::value_;
    return *this;
  }
};
