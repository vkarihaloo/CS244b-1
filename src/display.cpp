...
void ShowView(Loc x, Loc y, Direction dir) {
  ...
  for (ratIndex = 0; ratIndex < MAX_RATS;
...
hidden(Loc x, Loc y, Direction dir, XYpair *p) {
  ...
  edge1 = (edge3 && (edge2 || !M->maze_[l2x][l2y])) || ((!edge2) && edge4);
  edge5 = (edge7 && (edge2 || !M->maze_[r2x][r2y])) || ((!edge2) && edge6);
 ...
 */
static void TokenVisible(RatIndexType hisRatIndex) {
  ...
  while (!M->maze_[tx.value()][ty.value()]) {
    ...
    if ((tx == rat.x) && (ty == rat.y)) {
      ...
      ix12 = ix * 12;
      ratLook->x = (viewTable[ix12 + 3].p2.x + viewTable[ix12 + 10].p1.x) / 2;
      ratLook->y = (viewTable[ix12 + 3].p1.y + viewTable[ix12 + 3].p2.y) / 2;
 ...
 */
static void XORToken(RatIndexType hisRatIndex) {
  int size;
  int srcX, srcY;

  getRat(hisRatIndex, &srcX, &srcY, &size);
  DisplayRatBitmap(Rats2Display[hisRatIndex.value()].x.value() - size / 2,
                   Rats2Display[hisRatIndex.value()].y.value() - size / 2, size,
                   size, srcX, srcY);
}
 ...
 */
static void getRat(RatIndexType hisRatIndex, int *x, int *y, int *size) {
  int viewT = LEFT;
 ...
 */
void SetMyRatIndexType(RatIndexType ratIndex) {
  Rat myRat;
 ...
 */
void SetRatPosition(RatIndexType ratIndex, Loc x_loc, Loc y_loc,
                    Direction dir) {
 ...
 * Someone left.
 */
void ClearRatPosition(RatIndexType ratIndex)
...
void ShowPosition(Loc x_loc, Loc y_loc, Direction tdir) {
 ...
 */
void showMe(Loc x_loc, Loc y_loc, Direction dir) {
  register BitCell *bp;
  Rat rp;
 ...
 */
void clearPosition(RatIndexType ratIndex, Loc xClear, Loc yClear) {
  Rat rp;
  register int i;
...
/* ----------------------------------------------------------------------- */
void UpdateScoreCard(RatIndexType ratIndex) {
 ...
 */
void bitFlip(register BitCell *bits, int size) {
  int num, top, bot, i, j;
 ...
 */
void byteSwap(register BitCell *bits, int size) {
  register int i, j;
 ...
 */
void showMissile(Loc x_loc, Loc y_loc, Direction dir, Loc prev_x, Loc prev_y,
                 bool clear) {
  {
    register BitCell *bp;
