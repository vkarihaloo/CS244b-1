...
String default_resources[] = { "time_interval 500",

NULL, };

static XrmOptionDescRec opTable[] = { { "-mouse", "*mouseColor",
    XrmoptionSepArg, (caddr_t) NULL }, { "-time_interval", "*time_interval",
 ...
 * been done.
 */
void StartWindow(int ratWidth, int ratHeight) {
...
void NextEvent(MWEvent *event, int socket) {
fd_set fdmask;
XEvent xev;
...
#define	LeftButton	Button1
    case ButtonPress:
      event->eventType = 0;
      switch (((XButtonPressedEvent *) &xev)->button & 0xff) {
        case RightButton:
    ...
    case ButtonRelease:
      event->eventType = 0;
      switch (((XButtonReleasedEvent *) &xev)->button & 0xff) {
        case RightButton:
...
if (FD_ISSET(socket, &fdmask)) {
  ...
  cc = recvfrom(socket, (char*) event->eventDetail, sizeof(MW244BPacket), 0,
...
void HackMazeBitmap(Loc x, Loc y, BitCell *newBits) {
arrowImage->data = (char *) newBits;
XPutImage(dpy, mwWindow, copyGC, arrowImage, 0, 0,
          x.value() * 16 + MAZE_X_ORIGIN, y.value() * 16 + MAZE_Y_ORIGIN, 16,
          16);
}
...
void WriteScoreString(RatIndexType rat) {
char buf[64];
int leftEdge;
 ...
 */
void ClearScoreLine(RatIndexType rat) {
XClearArea(
  dpy,
  mwWindow,
  SCORE_X_ORIGIN,
  SCORE_Y_ORIGIN
      + rat.value()
          * (scoreFontInfo->max_bounds.ascent
              + scoreFontInfo->max_bounds.descent),
  SCORE_X_DIM,
  scoreFontInfo->max_bounds.ascent + scoreFontInfo->max_bounds.descent, False);
}
 ...
 */
void InvertScoreLine(RatIndexType rat) {
XFillRectangle(
  dpy,
  mwWindow,
  xorGC,
...
m = XCreateBitmapFromData(dpy, mwWindow, (char *) ratMask_bits,
ratMask_width,
                          ratMask_height);
p = XCreateBitmapFromData(dpy, mwWindow, (char *) rat_bits,
rat_width,
                          rat_height);
...
m = XCreateBitmapFromData(dpy, mwWindow, (char *) dRatMask_bits,
dRatMask_width,
                          dRatMask_height);
p = XCreateBitmapFromData(dpy, mwWindow, (char *) dRat_bits,
dRat_width,
                          dRat_height);
deadRatCursor = XCreatePixmapCursor(dpy, p, m, &mc, &bc, 0, 0);

p = XCreateBitmapFromData(dpy, mwWindow, (char *) cup_bits,
cup_width,
                          cup_height);
...
static Pixmap xCreateBitmapFromBitCell(Display *display, Drawable d, char *data,
                                       unsigned int width,
                                       unsigned int height) {
XImage ximage;
GC gc;
